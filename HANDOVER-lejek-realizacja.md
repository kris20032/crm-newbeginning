...
- Branch = v91, wszystko wypchnięte, working tree czysty. Zrobione Kroki 1–8 (lejek, usługi, nawigacja, **panel admina+RBAC**, „Baza partnerów", **katalog usług „Oferta"**, **redesign karty**, **token partnera + bramki lejka + checklista**).
...
Aktualna wersja cache: **v91** (w `index.html` przy `styles.css`/`config.js`/`app.js` — **przy każdej zmianie front podbij numer**, żeby zespół nie miał starej wersji z cache).
